...
if (options.projectmap) {
  options.projectmap = Object.fromEntries(
    options.projectmap.map((map) => {
      map = map.split(":");
      return [map[0], map[1]];
    })
  );
...
class MigrateProjectData {
  ...
  public async run() {
    await this.migrateObject("cohorts", (object) => {
      if (object.is_static) {
        console.warn(
          `Cohort with ID ${object.id} skipped, as it's a static cohort. Please move this one manually.`
        );
        return false;
      }
      return true;
    });
    await this.migrateObject("actions", null, (object) => {
    ...
    await this.migrateObject("experiments", null, (object) => {
      replaceCohortsRecurse(object.filters, this.state["cohorts"]);
      for (let i = 0; i < object.filters.actions?.length; i++) {
        object.filters.actions[i].id =
          this.state["actions"][object.filters.actions[i].id];
      }
      return object;
    });
    ...
    await this.migrateObject("dashboards", null, (object) => {
      replaceCohortsRecurse(object.filters, this.state["cohorts"]);
      return object;
    });
    await this.migrateObject("insights", null, (object) => {
      try {
        replaceCohortsRecurse(object.filters, this.state["cohorts"]);
      } catch (e) {
        return null;
      ...
      if (object.dashboard) {
        ...
        object.dashboards = object.dashboards
          .map((dashboardId) => {
    ...
    await this.migrateObject("annotations", null, (annotation) => {
      if (annotation.dashboard_item) {
        annotation.dashboard_item =
          this.state["insights"][annotation.dashboard_item];
      }
      return annotation;
    });
  }
  private async migrateObject(
    key: key,
    filterObjects?: (object) => boolean,
    mapObjects?: (object) => any
  ) {
    if (!this.state[key]) {
      this.state[key] = {};
    }
    ...
    for (let i = 0; i < allObjects.length; i++) {
      ...
      if (this.state[key][object.id]) {
        // Already moved this object over
        skipped += 1;
        continue;
      }
      let newItem;
      try {
        ...
        newItem = await destinationapi.method("post").create()({
          ...object,
          project_id: this.destinationId,
        });
        success += 1;
    ...
    console.log(
      `[${key}] ${success} moved, ${errors} errored, ${skipped} skipped`
    );
  ...
  private async paginate(api: any, key: key): Promise<Record<any, any>[]> {
    const response = await api({
      ...
      basic: true,
      ...(key === "insights"
        ? { order: "-last_modified_at", saved: true }
        : {}),
    });
    if (response.data.next) {
      return await this.recursePaginate(
        response.data.next,
...
async function run() {
  ...
  const sourceProjects = await source
    .path("/api/projects/")
    .method("get")
    .create()({});
  for (let i = 0; i < sourceProjects.data.results.length; i++) {
    ...
    if (!state.state.projects[project.id]) {
      ...
      if (options.projectmap[project.id]) {
        ...
        try {
          // Remove deprecated access_control field before creating project
          const { access_control, ...projectWithoutAccessControl } = project;
          newproject = await destination
            .path("/api/projects/")
            .method("post")
            .create()(projectWithoutAccessControl);
    ...
    const migrate = new MigrateProjectData(
      project.id,
      state.state.projects[project.id].destinationId,
      state
    );
    await migrate.run();
  }
